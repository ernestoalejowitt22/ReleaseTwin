...
Grounds "how would customers use it" in what's literally built today, not the aspirational version. Written to prep design-partner conversations honestly — see the initial design brief own warning: *"the mistake would be advertising the advanced workflow as five-minute setup."*
...
3. **What they see**: a case with a real HTTP call and a real JSONPath assertion, evidence and classification exactly as before.
...
5. **What they still can't do**: test something that isn't reachable over HTTP, or get Azure-DevOps-specific behavior beyond the fixed operations already built. Be upfront about this before they ask.
...
- "Testing your actual workflow is a scoped build-out, done together as part of the pilot" — this is what the initial design brief suggested pilot structure already assumes (6-8 weeks, one critical workflow, assisted setup).
- The pilot pricing range from the initial design brief ($5,000-$15,000) is a hypothesis to test, not a fixed quote.

**Don't say:**
- "It already works with your system" — it doesn't yet, for anyone but Azure DevOps, and only in the fixed-operation shape.
- Anything implying a five-minute setup for real release-proof coverage — the initial design brief is explicit that 1-2 engineering days is the honest target once fixtures, vendors, and flags are real.
...
Reused from the initial design brief, since they're still exactly the right questions and none have been asked of a real prospect yet:
...
Enthusiasm for the demo mechanics is not evidence of willingness to pay — the initial design brief own warning. The signal that matters is whether they'd commit budget to Tier 2 work on their actual workflow.
