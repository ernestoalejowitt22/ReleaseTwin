...
Cross-phase reference. Not scoped to any single change — every phase's design should stay compatible with this, even before a CLI or hosted control plane exists. Adapted from quik-testing's `docs/commercialization-assessment.md` (customer onboarding / deployment model sections) for ReleaseTwin's actual current state, not copied from it.
...
- **External-check connector (Playwright)** — visual/browser evidence isn't wired in as a *product* adapter (testing a customer's own UI). Separately: no browser-level (Playwright/Cypress) tests exist *of ReleaseTwin's own* `web/` frontend either — `hosted-react-frontend` deliberately didn't add that speculatively, matching the same "one dependency per real need" reasoning above; the hosted API's own test suite (`dotnet test`, in-process via `WebApplicationFactory`) is the closest thing to integration coverage that exists today.
- **Enterprise features** (SSO, audit logs, custom retention, private/on-prem hosted deployment) — per the assessment, these come after paid demand is demonstrated, not before.
