# release-rollup Specification

## Purpose

Aggregates a project's case and flag-proof reports by a case-declared `release` label into a per-release readiness view — the latest result per case, proven / failing / stale counts, and a single headline Proven / Not proven / Incomplete state — gated by the `releaseRollup` entitlement.
