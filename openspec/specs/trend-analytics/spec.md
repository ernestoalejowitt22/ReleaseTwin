...
Time-bucketed pass-rate, flag-proof-rate, run-volume, and failure-classification series over a selectable window — per project and per organization — plus a flakiest-cases list, all derived from existing uploaded report metadata and gated by the `trendAnalytics` entitlement.
