...
- Each cadence is a distinct Polar **product** (Polar's checkout API takes product ids; `product_price_id` is deprecated), so `PolarOptions.ProductIds` maps `(tier, cadence)` → product id. The webhook payload's `recurring_interval` → stored as `Organization.BillingCadence` for display ("renews monthly" / "renews 12 Jan 2027"). Tier mapping ignores cadence (both → Team).
