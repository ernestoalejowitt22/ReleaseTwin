...
- Apply the existing DynamoDB table terraform (`hosted/terraform/main.tf`) against a real AWS account — the `ealejo` account (846136340491), not the shared/unrelated `a separate shared account` account.
...
- Keep Clerk on its existing dev instance (`classic-marlin-8065.accounts.dev`, same keys already in `web/.env.local`) — no new Clerk application, no production Clerk migration.
...
- New: DynamoDB table actually provisioned in AWS (`ealejo` account), via the existing terraform.
