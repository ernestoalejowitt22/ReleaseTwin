...
- [ ] 1.1 Add tool + package metadata to `src/ReleaseTwin.Cli/ReleaseTwin.Cli.csproj`:
...
- [ ] 1.2 `dotnet pack src/ReleaseTwin.Cli -c Release` locally — confirm a
...
- [ ] 1.3 `releasetwin --help` / no-arg behavior and `${ENV_VAR}` interpolation
      verified through the installed tool against `examples/`.
- [ ] 1.4 Add `src/ReleaseTwin.Cli/README.md` (tool page): install line, the
...
- [ ] 1.5 `reuse lint` green after the new files.

## 2. Release automation

- [ ] 2.1 `.github/workflows/release.yml`: after the test step, add
...
- [ ] 3.2 `integrations/github-action/action.yml`: update the `image` default
      comment; no behavior change.
- [ ] 3.3 `docs/ci.md`: replace the literal `OWNER` / `VERSION` placeholders
...
- [ ] 4.1 New `docs/install.md`: the three paths side by side — Docker (no
...
- [ ] 4.2 `docs/installation-model.md`: drop "`dotnet tool`/NuGet and a GitHub
      Action wrapper are still deferred" — state all three exist; Homebrew /
      single-file binary remain deferred.
- [ ] 4.3 `README.md` "What's not built yet": update the packaging bullet to
      match — Docker + `dotnet tool` + Action done, Homebrew deferred.
- [ ] 4.4 `docs/ideas/deferred-backlog.md` item 1: narrow it to just Homebrew /
...
- [ ] 5.2 `web/` unaffected — no build needed; confirm no `web/` files touched.
- [ ] 5.3 `openspec validate cli-distribution --strict` passes.
