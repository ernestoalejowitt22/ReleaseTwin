...
- [x] 3.1 New `hosted/terraform-state-backend`: `aws_s3_bucket` (`releasetwin-terraform-state-ACCOUNT_ID`, versioned, public access blocked) + `aws_dynamodb_table` for locking. No backend block — this is the one layer that stays genuinely local-stated (see design.md Decisions/Risks for why that's accepted here specifically). `terraform validate` and `terraform plan` both pass clean — 4 resources.
...
- [x] 5.1 In the AWS Console (the operator’s account account, logged in normally — MFA here is just your regular login, not a CLI session dance): IAM → Users → Create user `releasetwin-bootstrap`, no console access, access-key-only credential type.
    ...
    "Statement": [
      {
        ...
        "Action": "s3:*",
        "Resource": [
          "arn:aws:s3:::releasetwin-terraform-state-ACCOUNT_ID",
          "arn:aws:s3:::releasetwin-terraform-state-ACCOUNT_ID/*"
        ]
      },
      {
        ...
        "Effect": "Allow",
        "Action": "dynamodb:*",
        "Resource": "arn:aws:dynamodb:us-east-1:ACCOUNT_ID:table/releasetwin-terraform-state-lock"
      },
      {
        "Sid": "OidcAndRole",
        "Effect": "Allow",
...
- [x] 5.5 Set the CI role ARN from 5.4 as the `AWS_DEPLOY_ROLE_ARN` repo variable (used by `deploy-hosted.yml`). Confirmed already set: `arn:aws:iam::ACCOUNT_ID:role/releasetwin-github-actions-deploy`.
...
- [x] 9.1 Sign up for real through the deployed Vercel URL with a real email (not a `+clerk_test@` throwaway) — confirm landing on a freshly-provisioned dashboard. Found and fixed a real bug along the way: the app had no local `/sign-up` route, so `<SignIn>`'s built-in "Sign up" link fell back to Clerk's hosted Account Portal, which had no way to redirect back into the app after registration — landed on Clerk's own generic "Start building" placeholder instead of the dashboard (and a direct `/sign-up` visit was a 404). Added `web/src/app/sign-up/[[...sign-up]]/page.tsx` (`<SignUp fallbackRedirectUrl="/dashboard" signInUrl="/sign-in" />`) and wired `signUpUrl="/sign-up"` into the `/sign-in` page, matching the existing `/sign-in` pattern. Deployed via push to `main` (Vercel auto-deploy); confirmed `/sign-up` returns 200 for real. Signing back in confirmed a freshly-provisioned dashboard — Free plan, 0 case reports, 0 flag-proof reports, no projects.
- [x] 9.2 Create a project; click "Connect GitHub"; authorize with a real GitHub account; confirm the connection shows on the dashboard. Created project "production". First attempt at "Connect GitHub" 500'd — found a second real bug: the Lambda's IAM role was never granted the `ssm:*`/`kms:*` permissions `PersistKeysToAWSSystemsManager` (Data Protection key-ring persistence, added by `hosted-adapter-credentials`) needs on the real-AWS path, so *any* code path creating a data protector failed, including the pre-existing `ConnectionStateService` used by the GitHub connection flow — not just adapter-credentials itself. This was a documented-but-never-applied manual step (design.md Migration Plan); fixed properly as a new terraform-managed `aws_iam_role_policy` in `hosted/terraform/lambda.tf` instead of a one-off console click, matching how the DynamoDB policy is already IaC-managed. Redeployed via `deploy-hosted.yml`; retried — GitHub's OAuth screen correctly requested the incremental `repo` scope ("Public and private"), authorized, repo picker now listed private repos too (confirming the earlier `github-oauth-private-repos` scope fix, which had never actually been deployed until this session), connected `ernestoalejowitt22/ReleaseTwin`. Dashboard confirms: "Connected to ernestoalejowitt22/ReleaseTwin (github)".
- [x] 9.3 Issue a token from the deployed dashboard. Real token issued (`rtw_b5340156…`), shown once with the copy-paste `export`/`dotnet run` instructions per the dashboard spec.
- [x] 9.4 From a local machine, `export RELEASETWIN_API_TOKEN=<issued token>` and `RELEASETWIN_API_URL=<Function URL>`, then `dotnet run --project src/ReleaseTwin.Cli -- examples/cases`. Ran for real: `PASS AUTH-CHAIN-DEMO-1`, `FAIL CLM-042 (Infrastructure)` (expected — no Azure DevOps credentials configured), `FLAGPROOF FLAGPROOF-DEMO-1 (Ineligible)` (expected, same reason), `PASS HTTP-DEMO-1`.
- [x] 9.5 Reload the deployed dashboard; confirm the run history and usage counters reflect the real upload. Confirmed: usage counter now shows 3 case reports (was 0); run history lists `HTTP-DEMO-1 PASS`, `CLM-042 FAIL (Infrastructure)`, `AUTH-CHAIN-DEMO-1 PASS`, each with a real upload timestamp — matches the documented shape exactly.
