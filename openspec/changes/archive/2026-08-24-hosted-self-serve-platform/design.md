...
See proposal.md - Why. This design works out the full shape: what the hosted service is made of, what crosses the trust boundary between customer infra and hosted infra, how self-serve billing actually functions, and — because this is the single largest commitment in the project so far — how to stage delivery so it doesn't repeat the "build distribution before validating demand" mistake the commercialization assessment explicitly warns against.
...
- Enterprise features (SSO, audit logs, custom retention, private deployment) — per the assessment, these come after paid demand is demonstrated, not before.
...
Free tier: limited monthly uploaded runs, one project, short retention (exact numbers are hypotheses, same status as the assessment's own pricing hypotheses — validate, don't assume). Paid tiers: more volume, longer retention, more seats. Metering unit is **uploaded case executions**, with a paired known-bad/known-good flag-proof run counted as one "release proof," per the assessment's explicit instruction never to meter per-assertion, per-retry, or per-screenshot (creates an incentive to under-test). Plan enforcement is webhook-driven off Stripe subscription events, not built as a custom billing engine.
...
Given this is the largest commitment in the project's history, and per the assessment's explicit warning against building distribution before validating demand:
...
- **[Risk] Building any of this before a design partner has confirmed the value proposition risks the exact mistake the assessment warns against.** → Mitigation: Stage 1 is intentionally the smallest thing that proves the self-serve loop works — not a commercial launch, not a marketing push, just the free path becoming real instead of aspirational.
