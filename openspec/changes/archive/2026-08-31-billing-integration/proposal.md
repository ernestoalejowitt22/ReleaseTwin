...
- **Secrets / config** (see `preflight.md` + `docs/billing.md`): `POLAR_API_TOKEN` + `POLAR_WEBHOOK_SECRET` as GitHub Actions repo secrets; `POLAR_TEAM_PRODUCT_MONTHLY` / `POLAR_TEAM_PRODUCT_ANNUAL` + the URL / flag vars as repo variables (checkout takes Polar product ids); webhook URL registered in the Polar dashboard (standing manual step — no code path to create it).
