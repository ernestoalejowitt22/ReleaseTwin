...
- **Sign-up already works today, via Clerk's hosted Account Portal.** `/sign-in`'s `<SignIn/>` widget's "Sign up" link points to `https://<slug>.clerk.accounts.dev/sign-up?...` — Clerk's own hosted domain, not a local `/sign-up` route (`web/src/app/layout.tsx`'s `<ClerkProvider>` sets no `signUpUrl` override, so Clerk falls back to its default). No new frontend route needs to be built; the registration spec needs to follow this cross-origin link and drive Clerk's own hosted form.
